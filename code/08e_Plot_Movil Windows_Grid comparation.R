...
# ruta <- "output/08_Plots/e. Grid Comparation/C1_RCP45_RB_MW.jpg" # Activar para RCP 4.5
ruta <-"output/08_Plots/e. Grid Comparation/C1_RCP85_RB_MW.jpg" # Activar para RCP 8.5
...
#### GENERACION DE ENVOLVENTE GCM-RCM ####
...
#### GENERACION DE ENVOLVENTE GCM-RCM + PR ####
...
#### GENERACION DE ENVOLVENTE GCM-RCM + Hmodels ####
...
graph <- ggplot() +
  geom_line(data=df_sum,aes(x=year,y = Qm3s_MW,group=name), color = "gray80", size = 0.15) +
  
  geom_line(data=df_mediam2,aes(x=year,y = Qm3s_mediam), color="red",size=0.25)+
  geom_ribbon(data=df_env2,aes(x=year,ymin = Qm3s_min, ymax = Qm3s_max,fill = "GCM-RCM + Pr"), alpha = 0.15) +
  
  geom_line(data=df_mediam3,aes(x=year,y = Qm3s_mediam),color="darkgreen",size=0.25)+
  geom_ribbon(data=df_env3,aes(x=year,ymin = Qm3s_min, ymax = Qm3s_max,fill="GCM-RCM + Hmodels"), alpha = 0.15) +
  
  geom_line(data=df_mediam1,aes(x=year,y = Qm3s_mediam),color="blue",size=0.25)+
  geom_ribbon(data=df_env1,aes(x=year,ymin = Qm3s_min, ymax = Qm3s_max,fill="GCM-RCM"), alpha = 0.25) +
  
  
  facet_grid(hmodel ~ source_pr) +
  theme_bw() +
  scale_fill_manual("Uncertainty range: ",
    values = c(
      "GCM-RCM" = "blue",
      "GCM-RCM + Pr" = "red",
      "GCM-RCM + Hmodels" = "darkgreen")) +
  
  labs(x = "Year", y = "Mean annual runoff (mm)") +
