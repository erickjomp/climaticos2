...
#### OUTPUT ####
file_df_parameters__GR2M_csv <- 
  "output/03_hydrological_model_cal_val/df_parameters__GR2M.csv"
file_df_simulations__GR2M_rds <- 
  "output/03_hydrological_model_cal_val/df_Qm3s_simulations__GR2M.rds"
file_df_simulations__GR2M_csv <- 
  "output/03_hydrological_model_cal_val/df_Qm3s_simulations__GR2M.csv"
file_df_performance__GR2M_csv <- 
  "output/03_hydrological_model_cal_val/df_performance__GR2M.csv"
...
for(pr_source in names(df_pr_m__all)[-1]){
  ...
  InputsModel <- 
    CreateInputsModel(FUN_MOD = RunModel_GR2M, DatesR = df_series$dates,
                      Precip = df_series$pr, PotEvap =df_series$etp)
  ...
  ## results preview
  plot(OutputsModel, Qobs = df_series$Qmm[Ind_run])
  
  
  #### efficiency criterions
  df_Qmm_sim <- tibble(dates = OutputsModel$DatesR,
                       Qmm_sim = OutputsModel$Qsim)
...
#### writing OUTPUT ####
write.csv(df_parameters, file_df_parameters__GR2M_csv,
          row.names = FALSE, quote = FALSE)
saveRDS(df_Qm3s_simulations, file_df_simulations__GR2M_rds)
write.csv(df_Qm3s_simulations, file_df_simulations__GR2M_csv,
          row.names = FALSE, quote = FALSE)
write.csv(df_performance, file_df_performance__GR2M_csv,
          row.names = FALSE, quote = FALSE)
