...
#### OUTPUTS ####
file_plot_precipmethod_1 <- "output/08_plots_results/fig_variability_dueto_precipestimation_1_RCP45.pdf"
file_plot_precipmethod_2 <- "output/08_plots_results/fig_variability_dueto_precipestimation_2_RCP45.pdf"
...
df_plot1 <- df_Qy_change45
stats_plot1 <- get_facet_stats(df_plot1, c("hmodel"), "source_pr")
minmax_plot1 <- get_minmax_lines(df_plot1, c("hmodel"), "source_pr")
...
df_plot2_alt_rcp45 %>%
  ...
  geom_text(data = stats_plot2_alt_rcp45, 
            ...
            color = "black") +
  facet_wrap(~ GCM, nrow = 5, scales = "free_y") +
  labs(y = "Mean annual runoff change (%)", 
...
# TWO ROWS AND FIVE COLUMNS FORMAT FOR THE SUBPLOT:
# theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 5.5),
#       axis.text.y = element_text(size = 6),
#       axis.title = element_text(size = 7),
#       strip.text = element_text(size = 3.5, margin = margin(1, 0.5, 1, 0.5)),
...
#       legend.key.size = unit(0.35, "cm"),
#       plot.title = element_text(hjust = 0.5, face = "bold", size = 8),
#       panel.spacing.x = unit(0.05, "lines"),          # ← CHANGE 1
#       panel.spacing.y = unit(0.15, "lines"),          # ← CHANGE 2
#       plot.margin = margin(1, 1, 1, 1),               # ← CHANGE 3
#       aspect.ratio = 1.2) +                           # ← CHANGE 4
#   guides(color = guide_legend(nrow = 1, byrow = TRUE))
# 
# ggsave(filename = file_plot_precipmethod_2_alt_rcp45,
#        width = 190, height = 120, units = "mm")
...
df_plot3 %>%
  ...
  geom_segment(data = minmax_plot3, aes(x = hmodel, xend = hmodel, 
               ...
               inherit.aes = FALSE, color = "grey30", linewidth = 0.5, alpha = 0.4) +
  # Horizontal line at minimum
  geom_segment(data = minmax_plot3, aes(x = as.numeric(hmodel) - 0.1, xend = as.numeric(hmodel) + 0.1, 
                                        y = min_val, yend = min_val),
               inherit.aes = FALSE, color = "grey70", linewidth = 0.5) +
  # Horizontal line at maximum
  geom_segment(data = minmax_plot3, aes(x = as.numeric(hmodel) - 0.1, xend = as.numeric(hmodel) + 0.1, 
...
df_plot4 %>%
  ...
  geom_segment(data = minmax_plot4, aes(x = GCM, xend = GCM, 
               ...
               inherit.aes = FALSE, color = "grey30", linewidth = 0.5, alpha = 0.4) +
  # Horizontal line at minimum
  geom_segment(data = minmax_plot4, aes(x = as.numeric(GCM) - 0.3, xend = as.numeric(GCM) + 0.3, 
                                        y = min_val, yend = min_val),
               inherit.aes = FALSE, color = "grey70", linewidth = 0.5) +
  # Horizontal line at maximum
  geom_segment(data = minmax_plot4, aes(x = as.numeric(GCM) - 0.3, xend = as.numeric(GCM) + 0.3, 
