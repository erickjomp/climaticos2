...
#### OUTPUT ####
file_series_Q_GR2M_proj__rds <- 
  "output/07_Q_projections/df_Q_GR2M_proj.rds"
file_series_Q_GR2M_proj__csv <- 
  "output/07_Q_projections/df_Q_GR2M_proj.csv"
...
lapply(names_source_pr, function(name_source_pr){
  ...
  for (model in names_models){
    ...
    for (sce_rcp in c("rcp45","rcp85")){
      ...
      InputsModel <- 
        CreateInputsModel(FUN_MOD = RunModel_GR2M, 
                          DatesR = as.POSIXct(df_series_BC$dates),
                          Precip = df_series_BC$pr, 
                          PotEvap =df_series_BC$PET)
      
      ## running model
      Param <- df_parameters[[name_source_pr]]
      RunOptions <- CreateRunOptions(FUN_MOD = RunModel_GR2M,
                                     InputsModel = InputsModel, 
                                     IndPeriod_Run = ind_run,
                                     IndPeriod_WarmUp = ind_ext_warmup
      )
...
#### writing OUTPUT ####
saveRDS(df_Qm3s_CORDEX_BC__GR2M, 
        file = file_series_Q_GR2M_proj__rds)
write.csv(df_Qm3s_CORDEX_BC__GR2M,
          file = file_series_Q_GR2M_proj__csv,
          quote = FALSE,
          row.names = FALSE)
